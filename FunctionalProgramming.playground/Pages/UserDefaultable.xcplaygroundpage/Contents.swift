...
extension UserDefaults {
	struct Account: BoolUserDefaultable, FloatUserDefaultable {
		private init() { }
...
UserDefaults.Account.set(2.0, forKey: .isUserLogged)

// UserDefaults cannot write on disk. When we try to read value in UserDeafaults, result has not been persisted.
// Get
let isUserLoggedIn = UserDefaults.Account.bool(forKey: .isUserLoggedIn)
let isUserLoggedInFloat = UserDefaults.Account.float(forKey: .isUserLogged)
